...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule,MatFormFieldModule,MatInputModule,MatButtonModule,MatSelectModule,MatOptionModule,MatCardModule,CommonModule,HttpClientModule,RouterModule],
...
export class RegisterComponent {
   ...
   constructor(private authService:AuthService,private toastr:ToastrService){
    this.adminForm = new FormGroup({
      ...
      contact_number:new FormControl("",[Validators.required, Validators.pattern("^[0-9]*$"),ExactLengthValidator.exactLengthValidator(10) ]),
  ...
  CollectData(){
  if(this.username && this.email && this.contact_number && this.address && this.password &&this.organization_id && this.role){
    this.authService.register(this.username.value,this.email.value, this.contact_number.value , this.address.value , this.password.value ,this.organization_id.value , this.role.value).subscribe({
      next: (response) => {
